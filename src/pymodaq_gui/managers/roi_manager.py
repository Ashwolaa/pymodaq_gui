...
from pymodaq_gui.plotting.items.roi import (
    RectROI,
    LinearROI,
    EllipseROI,
    CircularROI,
...
class ROIScalableGroup(GroupParameter):
    ...
    def addNew(self, typ=""):
        ...
        child_indexes = [
            int(par.name()[len(name_prefix) + 1 :]) for par in self.children()
        ]
    ...
    def makeChild(self, index, roi_type):
        child = {
            "name": ROIManager.roi_format(index),
            "type": "bool",
            "value": True,
            "removable": True,
            "renamable": False,
        ...
        return [
            {
                "name": "Color",
                "type": "color",
                "value": list(np.roll(ROIManager.color_list, index)[0]),
            },
            {
                "name": "zlevel",
                "title": "Z-level",
            ...
            [
                {
                    ...
                    "type": "list",
                    "value": roi_type,
                    "limits": ["RectROI", "EllipseROI", "CircularROI"],
                    "readonly": False,
                }
            ]
        )
...
class ROIManager(QObject):
    ...
    def __init__(self, viewer_widget=None, ROI_type="1D"):
        ...
        self.viewer_widget = viewer_widget  # PlotWidget or ImageWidget
    ...
    def remove_roi_programmatically(self, index: int):
        self.settings.child("ROIs").removeChild(
            self.settings.child("ROIs", roi_format(index))
        )
    ...
    def setupUI(self):
        ...
        self.save_ROI_pb = QAction(
            QIcon(QPixmap(":/icons/Icon_Library/save_ROI.png")), "Save ROIs"
        )
        self.load_ROI_pb = QAction(
            QIcon(QPixmap(":/icons/Icon_Library/load_ROI.png")), "Load ROIs"
        )
        self.clear_ROI_pb = QAction(
            QIcon(QPixmap(":/icons/Icon_Library/clear_ROI.png")), "Clear ROIs"
        )
        ...
        self.settings = Parameter.create(
            title="ROIs Settings", name="rois_settings", type="group", children=params
        )
    ...
    def make_ROI2D(self, roi_type, index, pos, size, **kwargs):
        ...
        if roi_type == "RectROI":
            roi = RectROI(
                index=index, pos=pos, size=size, name=roi_format(index), **kwargs
            )
    ...
    def copy_ROI(self, roi: ROI):
        """Method to copy a ROI and add it to the parameter tree and to the
        viewer widget. The method extracts the parameters of the copied ROI,
        create a new parameter, a new ROI and update it with the settings
        from the copied parameter
        Args:
        ...
        param_to_update = putils.iter_children_params(
            param_roi, [], filter_name=("roi_type",), filter_type=("group",)
    ...
    def update_use_channel(self, channels: List[str], index=None):
        """Function to update the selected channels. If no index is given,
        the channels are applied to all ROIs.
        ...
        if index is not None:
            param = self.settings.child("ROIs", roi_format(index),
                                        "use_channel")
            param.setValue(dict(all_items=channels, selected=channels))
        else:
            for ind in range(len(self)):
                param = self.settings.child("ROIs", roi_format(ind),
                                            "use_channel")
                param.setValue(dict(all_items=channels, selected=channels))
    ...
    def save_ROI(self):
        try:
            ...
            path = select_file(
                start_path=Path.home(), ext="xml", save=True,
...
class ROISaver:
    ...
    def set_file_roi(self, filename, show=True):
        ...
        self.roi_presets = Parameter.create(
            title="roi", name="rois", type="group", children=children
        )
        ...
        for ind_det, det_roi in enumerate(det_children):
            det_module = self.detector_modules[
                det_module_names.index(det_names[ind_det])
            ]
            viewer_children = [
                child for child in det_roi.children() if "viewer" in child.opts["name"]
            ]
            for ind_viewer, viewer in enumerate(det_module.viewers):
                rois_params = [
                    child
                    for child in viewer_children[ind_viewer].children()
                ...
                if len(rois_params) > 0:
                    if hasattr(viewer, "roi_manager"):
                        if hasattr(
                            viewer, "activate_roi"
                        ):  # because for viewer 0D it is irrelevant
                            viewer.activate_roi()
    ...
    def set_new_roi(self, file=None):
        ...
        for ind_det, det in enumerate(self.detector_modules):
            det_param = Parameter.create(
                name=f"det_{ind_det:03d}",
                type="group",
                children=[
                    {
                        "title": "Det Name:",
    ...
    def show_rois(self):
        ...
        if res == dialog.Accepted:
            ...
            ioxml.parameter_to_xml_file(
                self.roi_presets,
                os.path.join(roi_path, self.roi_presets.child("filename").value()),
            )
