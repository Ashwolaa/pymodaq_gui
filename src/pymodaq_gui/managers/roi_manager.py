...
class ROISaver(ConfigManager):
    ...
    def __init__(self, msgbox=False, det_modules=[]):
        ...
        super().__init__(config_path=roi_path, msgbox=msgbox)

    def set_config_from_file(self, file_path: Path, show=True):
        """Load an existing ROI configuration from an XML file."""
        super().set_config_from_file(file_path, show=False)

        det_children = [child for child in self.settings.children() if 'det' in child.opts['name']]
        det_names = [child.child('detname').value() for child in self.settings.children() if
    ...
    def set_file_roi(self, filename, show=True):
        ...
        logger.warning("set_file_roi() is deprecated, use set_config_from_file() instead")
        self.set_config_from_file(filename, show)

    def make_config(self):
        """Build ROI configuration from detector modules"""
        config_children = []
        for ind_det, det in enumerate(self.detector_modules):
            det_param = {'name': f'det_{ind_det:03d}', 'type': 'group', 'children': [
            ...
            for ind_viewer, viewer in enumerate(det.ui.viewers):
                ...
                if hasattr(viewer, 'roi_manager'):
                    ...
                    roi_children.extend([child.saveState() for child in viewer.roi_manager.settings.child('ROIs').children()])
                    viewer_child['children'].extend(roi_children)
                viewers_children.append(viewer_child)
            det_param['children'].extend(viewers_children)
            config_children.append(det_param)

        return config_children

    def set_new_roi(self, file=None):
        """Backward compatibility wrapper - DEPRECATED, use set_new_config() instead"""
        logger.warning("set_new_roi() is deprecated, use set_new_config() instead")
        return self.set_new_config(file, show=True)

    def show_rois(self):
        """Backward compatibility wrapper - DEPRECATED, use show_config() instead"""
        logger.warning("show_rois() is deprecated, use show_config() instead")
